...
@section('content')
  ...
  <div class="col-span-2 p-4 h-full bg-white rounded-xl flex flex-col gap-4">
    ...
    {{-- Filters --}}
    <form method="GET" action="{{ route('support.seo-audit.index') }}" class="mb-3 grid grid-cols-2 gap-3">
      <div>
    ...
    <div>
      ...
      <form method="POST" action="{{ route('support.seo-audit.store') }}" class="grid grid-cols-2 gap-3">
        @csrf
        ...
        <div>
          ...
          <input type="text" name="country" value="{{ old('country', 'NL') }}"
                 class="w-full py-2.5 px-3 rounded-xl border border-gray-200 text-sm text-[#215558] font-medium outline-none focus:border-[#3b8b8f] transition">
          @error('country')
            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
          @enderror
        ...
        <div>
          ...
          <input type="text" name="locale" value="{{ old('locale', 'nl-NL') }}"
                 class="w-full py-2.5 px-3 rounded-xl border border-gray-200 text-sm text-[#215558] font-medium outline-none focus:border-[#3b8b8f] transition">
          @error('locale')
            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
          @enderror
        ...
        <tbody>
          @forelse($audits as $audit)
            <tr class="border-b border-gray-50 hover:bg-gray-50/60 transition">
              ...
              <td class="py-2 pr-2 align-top">
                <p class="text-xs text-gray-700 font-medium">{{ $audit->domain }}</p>
              </td>
              <td class="py-2 pr-2 align-top">
