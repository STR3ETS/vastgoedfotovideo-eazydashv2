...
<div class="col-span-5 flex-1 min-h-0">
    ...
    <script>
        (function() {
            ...
            const getCached = (q) => {
                try {
                    return JSON.parse(localStorage.getItem(cacheKey(q)) || 'null');
                } catch (e) {
                    return null;
                }
            };
            const setCached = (q, latlng) => {
                try {
                    localStorage.setItem(cacheKey(q), JSON.stringify(latlng));
                } catch (e) {}
            };
