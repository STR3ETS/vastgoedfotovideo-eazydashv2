...
$view = request()->query('view', 'list'); // list | map | calendar
...
$statusPill = function (?string $status) {
    ...
    return match ($status) {
        ...
        // task statuses (als je die zo gebruikt)
        'pending' => ['label' => 'Open', 'class' => 'text-[#2A324B] bg-[#2A324B]/15'],
        'doing' => ['label' => 'Bezig', 'class' => 'text-[#87A878] bg-[#87A878]/15'],
...
$taskUrl = function ($task) {
    ...
    return $project ? route('support.projecten.taken.show', [$project, $task]) : '#';
};
...
$taskLocation = function ($task) {
    if (!empty($task->location)) return $task->location;

    $or = $task->project?->onboardingRequest;
    if (!$or) return '-';
...
$taskRegion = function ($task) {
    $or = $task->project?->onboardingRequest;
    if ($or && !empty($or->city)) return $or->city;

    $loc = (string) ($task->location ?? '');
    if (str_contains($loc, ',')) {
        $parts = array_map('trim', explode(',', $loc));
        return end($parts) ?: '-';
    }
    ...
    <div class="w-full p-8 bg-white border border-gray-200 rounded-2xl h-full min-h-0 flex flex-col">
        ...
        @if ($section === 'today')
            ...
            <div class="flex-1 min-h-0 bg-[#191D38]/5 rounded-2xl overflow-hidden flex flex-col">
                ...
                @php
                    $todayPlanning = ($todayRequests ?? collect())->map(function ($pi) {
                        return [
                            'type' => 'planning',
                            'model' => $pi,
                            'sort_at' => $pi->start_at ? Carbon::parse($pi->start_at) : Carbon::parse('9999-12-31'),
                        ];
                    });

                    $todayTaskRows = ($todayTasks ?? collect())->map(function ($task) {
                        $sort = $task->due_date ? Carbon::parse($task->due_date)->startOfDay() : Carbon::parse('9999-12-31');
                        return [
                            'type' => 'task',
                            'model' => $task,
                            'sort_at' => $sort,
                    ...
                    $todayRows = $todayPlanning->concat($todayTaskRows)->sortBy('sort_at')->values();
                @endphp

                <div class="flex-1 min-h-0 overflow-y-auto custom-scroll">
                    @if ($todayRows->isEmpty())
                        <div class="h-full w-full flex items-center justify-center px-6">
                            <p class="text-[#191D38]/70 text-sm font-semibold">Geen planningen of taken gevonden voor deze dag.</p>
                        </div>
                    @else
                            ...
                            @foreach ($todayRows as $row)
                                ...
                                @else
                                    @php
                                        ...
                                        $title = $task->name ?: 'Taak';
                                        $location = $taskLocation($task);
                                        $client = $projectClientName($project);

                                        $due = $task->due_date ? $formatDateNlShort($task->due_date) : '-';
                                        $who = $task->assignedUser?->name ?: 'Niet toegewezen';
                                        $pill = $statusPill($task->status ?? null);
                                    @endphp

                                    <div class="px-6 py-5">
                                        <div class="grid grid-cols-[1.7fr_1.1fr_1fr_0.6fr] items-center gap-6">
                                            <a href="{{ $taskUrl($task) }}" class="min-w-0 block">
                                                <div class="flex items-center gap-2 mb-2">
                                                    <span class="inline-flex items-center gap-2 text-xs font-semibold text-[#009AC3]">
                                                        <i class="fa-solid fa-list-check"></i> taak
                                                    </span>
                                                ...
                                                <p class="text-[#191D38] font-black text-base leading-tight truncate">{{ $title }}</p>
                                                <p class="text-[#191D38]/70 text-sm font-semibold mt-1 truncate">{{ $location }}</p>
                                                <p class="text-[#191D38]/60 text-xs font-semibold mt-2 truncate">{{ $who }}</p>
                                            </a>
                                            ...
                                            <div class="text-right min-w-0">
                                                <p class="text-[#191D38] font-semibold text-sm truncate">{{ $client }}</p>
                                                <span class="inline-flex items-center justify-center mt-2 px-3 py-1 rounded-full text-xs font-semibold bg-[#191D38]/10 text-[#191D38]">
                                                    Taak
                                                </span>
                                            </div>
                                        </div>
        ...
        @if ($section === 'planning')
            @php
                // Helpers (veilig binnen dit blok) — GEEN "use Carbon\Carbon;" in Blade
                $taskUrl = function ($task) {
                    return route('support.projecten.taken.show', [
                ...
                // Status pill uitbreiden voor taken (pending/doing/done)
                $statusPillSmart = function (?string $status) use ($statusPill) {
                    $s = strtolower(trim((string) $status));

                    // jouw bestaande project statuses
                    if (in_array($s, ['new', 'planned', 'done', 'cancelled', 'canceled'], true)) {
                        return $statusPill($s);
                    }
                    ...
                    return match ($s) {
                        'pending' => ['label' => 'Open',  'class' => 'text-[#2A324B] bg-[#2A324B]/15'],
                        'doing'   => ['label' => 'Bezig', 'class' => 'text-[#87A878] bg-[#87A878]/15'],
                        'done'    => ['label' => 'Klaar', 'class' => 'text-[#009AC3] bg-[#009AC3]/15'],
                        default   => ['label' => $s !== '' ? ucfirst($s) : 'Onbekend', 'class' => 'text-[#2A324B] bg-[#2A324B]/15'],
                    };
                };

                // Data
                $planningItemsCol = ($planningItems ?? collect());   // project_planning_items
                $planningTasksCol = ($planningTasks ?? collect());   // project_tasks (voor jou)

                // ✅ Groeperen per dag -> per project -> uitklappen met jouw taken
                // We maken "dayProjects": [dateKey => [ ['project'=>..., 'items'=>[], 'tasks'=>[] ], ... ] ]
                $rows = collect();

                foreach ($planningItemsCol as $pi) {
                    $dateKey = $pi->start_at ? \Carbon\Carbon::parse($pi->start_at)->toDateString() : 'zonder-datum';
                    ...
                    $rows->push([
                        ...
                        'model'      => $pi,
                        'sort_at'    => $pi->start_at ? \Carbon\Carbon::parse($pi->start_at)->timestamp : PHP_INT_MAX,
                    ]);
                }

                foreach ($planningTasksCol as $t) {
                    $dateKey = $t->due_date ? \Carbon\Carbon::parse($t->due_date)->toDateString() : 'zonder-datum';
                    ...
                    $rows->push([
                        ...
                        'model'      => $t,
                        'sort_at'    => $t->due_date ? \Carbon\Carbon::parse($t->due_date)->startOfDay()->timestamp : PHP_INT_MAX,
                    ]);
                }
                ...
                // date title
                $dayTitle = function ($key) use ($formatDateNlLong) {
                    if ($key === 'zonder-datum') return 'Zonder datum';
                    return ucfirst($formatDateNlLong($key));
                };

                // per dag -> per project
                $dayProjects = $dayGroups->map(function ($dayRows) {
                    return $dayRows
                        ->groupBy('project_id')
                        ->map(function ($projectRows) {
                            $project = $projectRows->first()['project'];

                            $items = $projectRows->where('kind', 'planning')->sortBy(function ($r) {
                                return $r['model']->start_at
                                    ? \Carbon\Carbon::parse($r['model']->start_at)->timestamp
                                    : PHP_INT_MAX;
                            })->values();

                            $tasks = $projectRows->where('kind', 'task')->sortBy(function ($r) {
                                // due_date heeft geen tijd, dus sorteer op status dan naam
                                $t = $r['model'];

                                $prio = match (strtolower((string) $t->status)) {
                                    'doing'   => 0,
                                    'pending' => 1,
                                    'done'    => 2,
                                    default   => 3,
                                ...
                                return $prio * 1000000 + crc32((string) $t->name);
                            })->values();

                            return [
                                'project' => $project,
                                'items'   => $items, // planning items
                                'tasks'   => $tasks, // jouw taken
                            ];
                        })
                        ->values();
                });

                // Als groupBy in de UI op photographer/region staat, laten we voorlopig "day->project" leidend.
            @endphp
                ...
                <div x-data="{ open:false }" class="relative inline-block">
                    <form x-ref="filterForm" method="GET" action="{{ $baseUrl }}" class="flex items-center gap-2">
                        ...
                        <button type="button"
                            ...
                            <span class="text-sm text-[#191D38] font-semibold group-hover:text-white transition duration-200">Filter op datum</span>
                        ...
                        <div x-show="open"
                            ...
                            style="display:none;">
                            <div class="p-2">
                                <p class="px-3 pt-2 pb-2 text-[11px] font-black text-[#191D38]/60 uppercase tracking-wide">Periode</p>
                                ...
                                <div class="space-y-1">
                                    @foreach($rangeOptions as $key => $opt)
                                        <button type="button"
                                                ...
                                                @click="$refs.range.value = '{{ $key }}'; $refs.filterForm.submit();">
                                ...
                                <p class="px-3 pt-1 pb-2 text-[11px] font-black text-[#191D38]/60 uppercase tracking-wide">Exacte datum</p>
                                ...
                                @if($hasActiveFilter)
                                    <div class="px-3 pb-3 pt-1">
                                        <a href="{{ $baseUrl . '?section=planning&view=' . $view . '&group_by=' . $groupBy . '&per_page=' . ($perPage ?? 10) }}"
            ...
            @if($view === 'calendar')
                ...
                <style>
                    ...
                    .planning-cal .fc .fc-scrollgrid, .planning-cal .fc .fc-scrollgrid table { border-color: rgba(25, 29, 56, .10) !important; }
                    .planning-cal .fc .fc-col-header-cell-cushion { padding: 10px 0 !important; font-weight: 900 !important; color: rgba(25, 29, 56, .55) !important; text-transform: lowercase; font-size: 12px !important; letter-spacing: .02em; }
                    .planning-cal .fc .fc-daygrid-day-number { font-weight: 800 !important; color: rgba(25, 29, 56, .85) !important; padding: 10px !important; font-size: 12px !important; }
                    .planning-cal .fc .fc-daygrid-day { background: #fff !important; }
                    .planning-cal .fc .fc-daygrid-day.fc-day-other { background: rgba(25, 29, 56, .02) !important; }
                    .planning-cal .fc .fc-day-today { background: rgba(0, 154, 195, .08) !important; }
                    .planning-cal .fc .fc-daygrid-event { border: 0 !important; background: transparent !important; padding: 0 !important; margin: 6px 8px 0 8px !important; }
                    .planning-cal .fc .fc-daygrid-event .fc-event-main { padding: 0 !important; }
                    .planning-cal .fc .fc-event-title, .planning-cal .fc .fc-event-time { font-weight: 800 !important; }
                    .planning-cal .fc .fc-more-link { margin: 6px 10px !important; font-weight: 800 !important; color: #009AC3 !important; }
                    ...
                    .planning-cal-dot { width: 8px; height: 8px; border-radius: 9999px; background: #009AC3; flex: 0 0 auto; }
                    .planning-cal-time { font-size: 11px; font-weight: 900; color: rgba(25, 29, 56, .55); flex: 0 0 auto; }
                    .planning-cal-title { font-size: 12px; font-weight: 900; color: #191D38; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
                    .planning-cal-event:hover { border-color: rgba(0, 154, 195, .35); background: rgba(0, 154, 195, .06); }
                </style>
                        ...
                        <div class="flex items-center justify-between gap-4 flex-wrap">
                            <div class="flex items-center gap-2">
                                <button type="button"
                                        ...
                                        onclick="window.__planningCalPrev && window.__planningCalPrev();" title="Vorige">
                                    <i class="fa-solid fa-chevron-left text-[#191D38] group-hover:text-white transition duration-200 text-sm"></i>
                                </button>

                                <button type="button"
                                        class="w-10 h-9 rounded-full border border-gray-200 bg-white flex items-center justify-center hover:bg-[#009AC3] hover:border-[#009AC3] group transition duration-300"
                                        onclick="window.__planningCalNext && window.__planningCalNext();" title="Volgende">
                                    <i class="fa-solid fa-chevron-right text-[#191D38] group-hover:text-white transition duration-200 text-sm"></i>
                                </button>
                    ...
                    (function() {
                        ...
                        const calendar = new FullCalendar.Calendar(el, {
                            ...
                            eventContent: function(arg) {
                                ...
                                const time = start ? new Intl.DateTimeFormat('nl-NL', { hour: '2-digit', minute: '2-digit' }).format(start) : '';
            ...
            @elseif($view === 'map')
                {{-- Map-view: laten we voorlopig zoals jij had (items los). Dit kun je later ook per project groeperen. --}}
                @php
                    $allMapRows = $rows->values();

                    $mapPoints = $allMapRows->map(function ($row, $idx) use ($projectTitle, $planningLocation, $taskLocation, $projectUrl, $taskUrl, $projectClientName, $formatTime, $formatDateNlShort) {
                        ...
                        return [
                            ...
                            'url' => $taskUrl($task),
                            'client' => $projectClientName($project),
                            'time' => $task->due_date ? ('Deadline: ' . $formatDateNlShort($task->due_date)) : 'Deadline: -',
                        ];
                    })->values();
                @endphp
                ...
                <style>
                    .nr-marker {
                        ...
                        background: #009AC3; border: 2px solid #fff;
                        box-shadow: 0 6px 16px rgba(25, 29, 56, .25);
                    }
                    .nr-marker--active { transform: scale(1.08); background: #191D38; }
                </style>
                        ...
                        <div x-show="open" x-transition class="bg-white/95 backdrop-blur rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                            <div class="px-5 py-4 border-b border-gray-200">
                                <div class="flex items-center justify-between gap-3">
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-[#191D38]/70 text-xs font-semibold mt-1 truncate">
                                            {{ $mapPoints->count() }} items met markers
                                        </p>
                                    </div>
                            ...
                            <div class="max-h-[38vh] overflow-y-auto custom-scroll divide-y divide-[#191D38]/10">
                                @if($mapPoints->isEmpty())
                                    <div class="px-5 py-5">
                                        <p class="text-[#191D38]/70 text-sm font-semibold">Geen items gevonden.</p>
                                    </div>
                                @else
                                    @foreach($mapPoints as $p)
                                        <button type="button"
                                                class="w-full text-left px-5 py-4 hover:bg-white transition"
                                                onclick="window.__planningMapFocus && window.__planningMapFocus('{{ $p['type'] }}:{{ $p['id'] }}');">
                ...
                <script>
                    (function() {
                        ...
                        const getCached = (q) => { try { return JSON.parse(localStorage.getItem(cacheKey(q)) || 'null'); } catch (e) { return null; } };
                        const setCached = (q, latlng) => { try { localStorage.setItem(cacheKey(q), JSON.stringify(latlng)); } catch (e) {} };
                        ...
                        const fitAll = () => {
                            ...
                            if (!layers.length) { map.setView([52.1326, 5.2913], 7); return; }
                            map.fitBounds(markerGroup.getBounds(), { padding: [60, 60] });
                        };

                        window.__planningMapFocus = (key) => {
                        ...
                        const run = async () => {
                            ...
                            for (let i = 0; i < points.length; i++) {
                                ...
                                const cached = getCached(q);
                                if (cached && cached.lat && cached.lon) { addMarker(p, cached.lat, cached.lon); continue; }
            ...
            @else
                {{-- ✅ LIST VIEW: per dag -> per project -> uitklap taken (jouw wens) --}}
                <div class="flex-1 min-h-0 bg-[#191D38]/5 rounded-2xl overflow-hidden flex flex-col">
                    <div class="shrink-0 px-6 py-4 bg-white/60 border-b border-[#191D38]/10">
                        <div class="grid grid-cols-[1.7fr_1.1fr_1fr] items-center gap-6">
                            <p class="text-[#191D38] font-bold text-xs opacity-50">Project</p>
                            <p class="text-[#191D38] font-bold text-xs opacity-50">Planning</p>
                            <p class="text-[#191D38] font-bold text-xs opacity-50 text-right">Klant</p>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 min-h-0 overflow-y-auto custom-scroll">
                        @if($dayProjects->isEmpty())
                            <div class="h-full w-full flex items-center justify-center px-6">
                                <p class="text-[#191D38]/70 text-sm font-semibold">Geen planningen of taken gevonden.</p>
                            </div>
                        @else
                            <div class="divide-y divide-[#191D38]/10">
                                @foreach($dayProjects as $dateKey => $projects)
                                    <div class="px-6 py-3 bg-white/40 border-b border-[#191D38]/10">
                                        <p class="text-[#191D38] font-black text-sm">{{ $dayTitle($dateKey) }}</p>
                                    </div>

                                    <div class="px-6 py-4 space-y-3">
                                        @foreach($projects as $pIndex => $bundle)
                                            @php
                                                $project = $bundle['project'];
                                                $title = $projectTitle($project);
                                                $client = $projectClientName($project);
                                                $pillProject = $statusPillSmart($project?->status ?? null);

                                                $items = $bundle['items'];
                                                $tasks = $bundle['tasks'];

                                                // kleine samenvatting rechts
                                                $firstItem = $items->first()['model'] ?? null;
                                                $timeSummary = '-';
                                                if ($firstItem && $firstItem->start_at) {
                                                    $timeSummary = trim($formatTime($firstItem->start_at) . ' - ' . $formatTime($firstItem->end_at));
                                                } elseif ($tasks->count()) {
                                                    $timeSummary = 'Taken: ' . $tasks->count();
                                                }

                                                $locSummary = '-';
                                                if ($items->count()) {
                                                    $locSummary = $planningLocation($items->first()['model']);
                                                } elseif ($tasks->count()) {
                                                    $locSummary = $taskLocation($tasks->first()['model']);
                                                }

                                                $openKey = $dateKey . ':' . (string)($project?->id ?? $pIndex);
                                            @endphp

                                            <div x-data="{ open: false }" class="bg-white border border-gray-200 rounded-2xl overflow-hidden">
                                                {{-- Header rij (1 project, niet 4 taken los) --}}
                                                <button type="button"
                                                        @click="open = !open"
                                                        class="w-full text-left px-5 py-4 hover:bg-[#191D38]/[0.02] transition">
                                                    <div class="grid grid-cols-[1.7fr_1.1fr_1fr] items-center gap-6">
                                                        <div class="min-w-0">
                                                            <div class="flex items-center gap-2 mb-2">
                                                                <span class="inline-flex items-center gap-2 text-xs font-semibold text-[#009AC3]">
                                                                ...
                                                                <span class="{{ $pillProject['class'] }} inline-flex items-center justify-center px-3 py-1 rounded-full text-[11px] font-semibold">
                                                                    {{ $pillProject['label'] }}
                                                                </span>

                                                                {{-- Chips met aantallen --}}
                                                                @if($items->count())
                                                                    <span class="inline-flex items-center justify-center px-3 py-1 rounded-full text-[11px] font-semibold bg-[#009AC3]/10 text-[#009AC3]">
                                                                        {{ $items->count() }} planning
                                                                    </span>
                                                                @endif
                                                                @if($tasks->count())
                                                                    <span class="inline-flex items-center justify-center px-3 py-1 rounded-full text-[11px] font-semibold bg-[#191D38]/10 text-[#191D38]">
                                                                        {{ $tasks->count() }} taken
                                                                    </span>
                                                                @endif
                                                            </div>

                                                            <p class="text-[#191D38] font-black text-base leading-tight truncate">
                                                                {{ $title }}
                                                            </p>

                                                            <p class="text-[#191D38]/70 text-sm font-semibold mt-1 truncate">
                                                                {{ $locSummary }}
                                                            </p>
                                                        </div>
                                                        ...
                                                        <div class="text-[#191D38] font-semibold text-sm">
                                                            {{ $timeSummary }}
                                                            <span class="ml-2 text-[#191D38]/50 text-xs font-black">
                                                                <i class="fa-solid fa-chevron-down" :class="open ? 'rotate-180' : ''" class="transition"></i>
                                                            </span>
                                                        </div>
                                                ...
                                                {{-- Uitklap: jouw planning items + jouw taken --}}
                                                <div x-show="open" x-transition class="border-t border-gray-200 bg-white">
                                                    <div class="px-5 py-4 space-y-4">

                                                        {{-- Snelle acties --}}
                                                        <div class="flex items-center justify-between gap-3 flex-wrap">
                                                            <p class="text-[#191D38] font-black text-sm">Jouw items</p>
                                                            <a href="{{ $projectUrl($project) }}"
                                                            class="inline-flex items-center gap-2 px-3 h-8 rounded-full border border-gray-200 bg-white text-xs font-semibold text-[#191D38] hover:bg-[#009AC3] hover:text-white hover:border-[#009AC3] transition">
                                                                Open project
                                                        ...
                                                        {{-- Planning items --}}
                                                        @if($items->count())
                                                            <div class="bg-[#191D38]/[0.03] rounded-2xl border border-[#191D38]/10 overflow-hidden">
                                                                <div class="px-4 py-3 bg-white/70 border-b border-[#191D38]/10">
                                                                    <p class="text-[#191D38] font-black text-xs uppercase tracking-wide opacity-60">Planning</p>
                                                                </div>
                                                                <div class="divide-y divide-[#191D38]/10">
                                                                    @foreach($items as $r)
                                                                        @php
                                                                            $pi = $r['model'];
                                                                            $t1 = $formatTime($pi->start_at);
                                                                            $t2 = $formatTime($pi->end_at);
                                                                            $loc = $planningLocation($pi);
                                                                            $ass = $pi->assignee?->name ?: 'Niet toegewezen';
                                                                        @endphp
                                                                        <div class="px-4 py-3">
                                                                            <div class="flex items-start justify-between gap-3">
                                                                                <div class="min-w-0">
                                                                                    ...
                                                                                    <p class="text-[#191D38]/70 text-sm font-semibold mt-1 truncate">{{ $loc }}</p>
                                                                                    <p class="text-[#191D38]/60 text-xs font-semibold mt-2 truncate">{{ $ass }}</p>
                                                                                </div>
                                                                                <span class="inline-flex items-center justify-center px-3 py-1 rounded-full text-[11px] font-semibold bg-[#009AC3]/15 text-[#009AC3]">
                                                                                    Planning
                                                                                </span>
                                                                            </div>
                                                                        </div>
                                                        ...
                                                        @if($tasks->count())
                                                            <div class="bg-[#191D38]/[0.03] rounded-2xl border border-[#191D38]/10 overflow-hidden">
                                                                <div class="px-4 py-3 bg-white/70 border-b border-[#191D38]/10">
                                                                    <p class="text-[#191D38] font-black text-xs uppercase tracking-wide opacity-60">Taken</p>
                                                                </div>
                                                                ...
                                                                <div class="divide-y divide-[#191D38]/10">
                                                                    @foreach($tasks as $r)
                                                                        @php
                                                                            $task = $r['model'];
                                                                            $pillT = $statusPillSmart($task->status ?? null);
                                                                            $locT  = $taskLocation($task);
                                                                            $dueT  = $task->due_date ? $formatDateNlShort($task->due_date) : '-';
                                                                        @endphp

                                                                        <a href="{{ $taskUrl($task) }}" class="block px-4 py-3 hover:bg-white/60 transition">
                                                                            <div class="flex items-start justify-between gap-3">
                                                                                <div class="min-w-0">
                                                                                    <div class="flex items-center gap-2">
                                                                                        <p class="text-[#191D38] font-black text-sm truncate">
                                                                                            {{ $task->name ?: 'Taak' }}
                                                                                        </p>
                                                                                        <span class="{{ $pillT['class'] }} inline-flex items-center justify-center px-3 py-1 rounded-full text-[11px] font-semibold">
                                                                                            {{ $pillT['label'] }}
                                                                                        </span>
                                                                                    </div>

                                                                                    <p class="text-[#191D38]/70 text-sm font-semibold mt-1 truncate">{{ $locT }}</p>
                                                                                    <p class="text-[#191D38]/60 text-xs font-semibold mt-2">Deadline: {{ $dueT }}</p>
                                                                                </div>

                                                                                <span class="shrink-0 inline-flex items-center justify-center px-3 py-1 rounded-full text-[11px] font-semibold bg-[#191D38]/10 text-[#191D38]">
                                                                                    Open taak
                                                                                </span>
                                                                            </div>
                                                                        </a>
                                                        ...
                                                        @if(!$items->count() && !$tasks->count())
                                                            <p class="text-[#191D38]/70 text-sm font-semibold">
                                                                Geen items voor jou binnen dit project op deze dag.
                                                            </p>
                                                        @endif
                                                    </div>
                                                </div>
        ...
        {{-- QC --}}
        @if ($section === 'qc')
            <div class="flex-1 min-h-0 bg-[#191D38]/5 rounded-2xl p-8 flex items-center justify-content-center">
